    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-6 bg-white border rounded-xl shadow-sm">
          <p className="text-gray-500">Total Profiles</p>
          <p className="text-4xl font-bold text-blue-600">{stats.counts.profiles}</p>
        </div>
        <div className="p-6 bg-white border rounded-xl shadow-sm">
          <p className="text-gray-500">System Users</p>
      ...
      <ul className="divide-y border rounded-lg bg-white">
        {stats.recent.map((p: any) => (
          <li key={p.id} className="p-4 flex justify-between">
            <span>{p.name}</span>
            <span className="text-gray-400 text-sm">{new Date(p.created_at).toLocaleDateString()}</span>
          </li>
        ))}
      </ul>
    </div>
